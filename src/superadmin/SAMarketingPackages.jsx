...
function SAMarketingPackages() {
  ...
  // EXPANDED FEATURE FLAGS - Now includes all available features
  const FEATURE_FLAGS = [
    { key:'csvExport',        label:'CSV Data Export' },
    { key:'firearmRegister',  label:'Firearm Register Module' },
    { key:'whiteLabelMode',   label:'White-Label Branding' },
    { key:'invoiceWatermark', label:'Remove Invoice Watermark' },
    { key:'clientList',       label:'Client List & Invoicing' },
    { key:'multiCurrency',    label:'Multi-Currency Support' },
    { key:'multiLanguage',    label:'Multi-Language Support' },
    { key:'autoBackup',       label:'Auto Email Backup' },
    { key:'marketingPage',    label:'Marketing Page' },
    { key:'trailsModule',     label:'Trails Module' },
    ...
    try {
      ...
      if (editPkg) {
        ({ error } = await supabase
          ...
          .update(payload)
          .eq('id', editPkg.id));
        if (!error) showToast('Package updated', 'success');
      } else {
        ({ error } = await supabase
          .from('marketing_packages')
          .insert([payload]));
        if (!error) showToast('Package created', 'success');
      }
  ...
  async function toggleActive(id, active) {
    try {
      ...
      showToast(`Package ${!active ? 'activated' : 'deactivated'}`, 'success');
      load();
    } catch (error) {
  ...
  return (
    <div>
      ...
      {loading ? (
        <div style={{color:'#6b7280',padding:40,textAlign:'center'}}>Loading packages...</div>
      ) : (
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(320px,1fr))',gap:16}}>
          {packages.map(pkg => {
            ...
            return (
              <div key={pkg.id} style={{
                ...
                borderRadius:12,
                padding:20,
                border:`1px solid ${pkg.active ? '#D4A85444' : '#222'}`,
                display:'flex',
                ...
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
                  ...
                  <span style={{
                    background:pkg.active?'#22c55e22':'#33333322',
                    color:pkg.active?'#22c55e':'#6b7280',
                    borderRadius:999,
                    padding:'2px 9px',
                    fontSize:11,
                    fontWeight:700,
                ...
                <div style={{color:'#D4A853',fontWeight:900,fontSize:22}}>
                  ...
                  {pkg.price > 0 && (
                    <span style={{fontSize:13,fontWeight:400,color:'#6b7280'}}>/{pkg.billing_cycle}</span>
                  )}
                </div>
                
                {pkg.description && (
                  <div style={{color:'#9ca3af',fontSize:13}}>{pkg.description}</div>
                )}
                ...
                <div style={{display:'flex',gap:8,marginTop:4}}>
                  ...
                  <button 
                    ...
                    style={{
                      flex:1,
                      background:'#222',
                      color:pkg.active?'#f59e0b':'#22c55e',
          ...
          {packages.length === 0 && (
            <div style={{color:'#6b7280',padding:24,textAlign:'center',gridColumn:'1/-1'}}>
              No marketing packages yet. Click "New Package" to create one.
            </div>
          )}
        </div>
      )}
        ...
        <div style={{
          ...
          <div style={{
            ...
            <form onSubmit={save} style={{display:'flex',flexDirection:'column',gap:14}}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
                <div style={{gridColumn:'1/-1'}}>
                  ...
                  <input
                    ...
                    placeholder="e.g. Safari Starter"
                    style={{
                      background:'#1a1a1a',
                      border:'1px solid #333',
                      color:'white',
              ...
              <div>
                <div style={{borderTop:'1px solid #222',paddingTop:14,marginBottom:10}}>
                  ...
                  <div style={{color:'#6b7280',fontSize:11,marginTop:2}}>
                    Leave blank for unlimited (∞). Set 0 to disable.
                  </div>
                </div>
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px 16px'}}>
                  {LIMIT_FIELDS.map(f => (
                    <div key={f.key}>
                      <label style={{color:'#9ca3af',fontSize:11,display:'block',marginBottom:3}}>
                        {f.label}
                      </label>
                      <input
                        type="number"
              ...
              {/* ─ Feature Flags - Now with all options ─ */}
              <div>
                <div style={{
                  color:'#D4A853',
                  fontSize:12,
                    ...
                    return (
                      <label
                        ...
                        style={{
                          ...
                          alignItems:'center',
                          gap:8,
                          background: on ? '#1e3a5f' : '#1a1a1a',
                          border: `1px solid ${on ? '#D4A853' : '#333'}`,
                          borderRadius:8,
                          padding:'8px 10px',
                          cursor:'pointer',
                          userSelect:'none'
                      ...
                      >
                        <div style={{
                          ...
                          height:16,
                          borderRadius:4,
                          border: `2px solid ${on ? '#D4A853' : '#555'}`,
                          background: on ? '#D4A853' : 'transparent',
                          display:'flex',
                          alignItems:'center',
                          justifyContent:'center',
                          flexShrink:0,
              ...
              <div>
                <div style={{borderTop:'1px solid #222',paddingTop:14,marginBottom:8}}>
                  ...
                  <div style={{color:'#6b7280',fontSize:11,marginTop:2}}>
                    Optional additional slots/modules bundled on top of base limits.
                  </div>
                </div>
                        ...
                        return (
                          <label
                            ...
                            style={{
                              ...
                              alignItems:'center',
                              gap:8,
                              background: chk ? '#1a2a1a' : '#161616',
                              border: `1px solid ${chk ? '#22c55e44' : '#2a2a2a'}`,
                              borderRadius:7,
                              padding:'7px 9px',
                              cursor:'pointer',
                              userSelect:'none'
                          ...
                          >
                            <div style={{
                              ...
                              height:14,
                              borderRadius:3,
                              border: `2px solid ${chk ? '#22c55e' : '#444'}`,
                              background: chk ? '#22c55e' : 'transparent',
                              display:'flex',
                              alignItems:'center',
                              justifyContent:'center',
                              flexShrink:0,
                            ...
                            <span style={{
                              color: chk ? '#22c55e' : '#9ca3af',
                              fontSize:11,
                              fontWeight: chk ? 600 : 400
                            }}>
                              {opt.label}
                            </span>
                          </label>
              ...
              <div style={{display:'flex',alignItems:'center',gap:8}}>
                ...
                />
                <label htmlFor="pkg-active" style={{color:'#9ca3af',fontSize:13,cursor:'pointer'}}>
