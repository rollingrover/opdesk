...
function BillingPage() {
  ...
  const badgeColor = { free:'badge-gray', basic:'badge-blue', standard:'badge-purple', premium:'badge-navy' };

  const SLOT_RESOURCES = ['vehicles','guides','drivers','shuttles','safaris','tours','charters','trails','seats','schedules'];
  const SLOT_LABELS    = { vehicles:'Vehicles', guides:'Guides', drivers:'Drivers', shuttles:'Shuttles',
    safaris:'Safaris', tours:'Tours', charters:'Charters', trails:'Trails', seats:'User Seats', schedules:'Schedules' };
  ...
  return (
    <div>
      ...
      {(() => {
        ...
        const basicPrice = TIERS['basic'].price;
              ...
              <div>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <span className={`badge ${isFreePlus ? 'badge-green' : badgeColor[tier]} text-base px-4 py-1`}>
                    {isFreePlus ? 'Free+' : TIERS[tier].label}
                  </span>
                  <span className="text-gray-500 text-sm">
                    {tier==='free'
                      ...
                      : `R${TIERS[tier].price.toLocaleString()}/month`}
                  </span>
                </div>
                {showNudge && (
      ...
      <div className="card p-6 mb-5">
        ...
        <p className="text-sm text-gray-400 mb-4">Base plan limits · purchased extras shown separately · click Buy to add more{tier==='free' ? ' · On Free, each slot is charged at add-on rates' : ''}</p>
            ...
            return (
              <div key={key} className="flex items-center gap-3 py-2 border-b border-gray-50 last:border-0">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  <span className="text-xs text-gray-400">R{price}/mo per slot</span>
                  <button
      ...
      {addons.length > 0 && (
        <div className="card p-6 mb-5">
          ...
          <table>
            <thead>
              <tr><th>Type</th><th>Qty</th><th>Price/mo</th><th>Total/mo</th><th>Added</th></tr>
            </thead>
            <tbody>
              {addons.map(a => (
                <tr key={a.id}>
                  <td className="capitalize font-medium">{SLOT_LABELS[a.addon_key] || a.addon_key}</td>
                  <td>{a.quantity || 1} slot{(a.quantity || 1) > 1 ? 's' : ''}</td>
                  <td className="text-gray-500">R{a.price_per_unit || 0}</td>
                  <td className="font-semibold">R{((a.price_per_unit || 0) * (a.quantity || 1)).toLocaleString()}</td>
                  <td className="text-gray-400 text-xs">{a.created_at ? new Date(a.created_at).toLocaleDateString('en-ZA') : '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="flex items-center justify-between py-3 border-b border-gray-100">
            ...
            <div className="flex items-center gap-3 flex-shrink-0 ml-4">
              {addons.some(a => a.addon_key === 'no_watermark' && a.active) ? (
                ...
                <>
                  <span className="text-xs text-gray-400">R49/mo</span>
                  <button
                    className="text-xs px-3 py-1 bg-navy text-white rounded-full hover:bg-navy/80 font-semibold"
      ...
      {addonResource && <AddOnPurchaseModal resource={addonResource} onClose={()=>{ setAddonResource(null); reloadAddons(); }}/>}
    </div>
  );
}
