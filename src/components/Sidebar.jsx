...
import { TIERS, CURRENCIES, LANGUAGES, COUNTRIES, DAYS, CATS, ICONS_MAP, LABELS, ADDON_TYPES, Logo, Icon } from '../lib/constants.jsx';
import AddOnPurchaseModal from './AddOnPurchaseModal.jsx';
...
function Sidebar({ page, setPage, collapsed, setCollapsed }) {
  ...
  const nav = [
    ...
    null,
    ...(tier !== 'free' || (addons && addons.some(a => a.addon_key === 'marketing_page' && a.active)) 
      ? [{ id:'marketing', label:'Marketing Page', icon:'marketing' }] 
      : []),
  ];
    ...
    <>
      <div className={`flex flex-col h-screen fixed left-0 top-0 z-30 ${collapsed ? 'w-16' : 'w-56'}`} style={{background:'#0F2540', transition:'width 0.2s'}}>
        <div className="flex items-center gap-3 px-4 py-5 cursor-pointer" onClick={() => setCollapsed(!collapsed)}>
          ...
          {!collapsed && (
            <div>
              <div className="font-black text-white text-lg leading-none">OpDesk</div>
              <div className="text-xs font-semibold tracking-widest uppercase leading-none mt-0.5" style={{color:'#D4A853'}}>
                Command Centre
              </div>
            </div>
          )}
        ...
        {!collapsed && (
          <div className="px-4 pb-3 border-b" style={{borderColor:'rgba(255,255,255,0.1)'}}>
            <p className="text-xs truncate" style={{color:'rgba(255,255,255,0.5)'}}>{company?.name}</p>
          </div>
        )}

        <nav className="flex flex-col flex-1 overflow-y-auto py-2 px-2">
          {nav.map((item, i) => {
        ...
        <div className="px-3 py-3 border-t" style={{borderColor:'rgba(255,255,255,0.1)'}}>
          {!collapsed && (
            <div className="flex items-center justify-between mb-2">
              <span className={`badge ${tier === 'free' && addons && addons.some(a => a.active) ? 'badge-green' : tierBadge[tier]} text-xs`}>
                {sym[tier]} {tier === 'free' && addons && addons.some(a => a.active) ? 'Free+' : TIERS[tier]?.label || tier}
              </span>
              {tier !== 'premium' && (
                <button 
          ...
          {!collapsed && (
            <div className="mb-2">
              <div className="text-xs truncate" style={{color:'rgba(255,255,255,0.5)'}}>{profile?.full_name}</div>
              <div className="text-xs capitalize" style={{color:'rgba(255,255,255,0.3)'}}>{role}</div>
            </div>
          )}
          <button 
            onClick={signOut} 
            className={`flex items-center gap-2 text-xs ${collapsed ? 'justify-center w-full' : ''}`} 
